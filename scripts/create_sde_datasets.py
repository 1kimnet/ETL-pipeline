...
def create_sde_datasets(
    ...
    try:
        ...
        log.error("❌ Failed to create spatial reference for EPSG:%d: %s", target_srid, exc)
        raise
    ...
    for dataset_name in datasets:
        try:
            dataset_path = f"{sde_connection}\\{dataset_name}"
    ...
    if error_count > 0:
        log.warning("⚠️ Some datasets failed to create. Check SDE permissions and connection.")
    else:
