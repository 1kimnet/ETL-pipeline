...
def ensure_unique_name(
    base_name: str, used_names: Set[str], max_length: int = 128
) -> str:
    ...
    if not candidate:
        raise ValueError(
            f"Base name '{base_name}' resulted in empty string after sanitization"
        )
    ...
    while final_candidate.lower() in (n.lower() for n in used_names):
        ...
        if available_length <= 0:
            raise ValueError(
                f"Cannot generate unique name for '{base_name}' within {max_length} characters"
            )
        ...
        if idx > 9999:
            raise ValueError(
                f"Could not find unique name for '{base_name}' after {idx - 1} attempts"
            )
...
def _remove_existing_gdb(gdb_full_path: Path) -> None:
    ...
    try:
        ...
        log.error(
            "❌ Failed to remove existing GDB '%s': %s",
            gdb_full_path,
            removal_error,
            exc_info=True,
        )
        raise RuntimeError(
            f"Failed to remove existing GDB '{gdb_full_path}': {removal_error}"
        ) from removal_error
...
def _ensure_parent_directory_exists(gdb_path: Path) -> None:
    ...
    if not parent_dir.exists():
        log.info(
            "🆕 Parent directory %s for GDB does not exist. Attempting to create.",
            parent_dir,
        )
        try:
            ...
            log.error(
                "❌ Failed to create parent directory '%s' for GDB: %s",
                parent_dir,
                dir_creation_error,
                exc_info=True,
            )
            raise RuntimeError(
                f"Failed to create parent directory '{parent_dir}' for GDB: {dir_creation_error}"
            ) from dir_creation_error


def _create_new_gdb(gdb_path: Path) -> None:
    """🆕 Create a new FileGDB with error handling."""
    log.info(
        "🆕 Attempting to create new FileGDB: %s in folder %s",
        gdb_path.name,
        gdb_path.parent.resolve(),
    )
    try:
        ...
        log.error(
            "❌ arcpy.management.CreateFileGDB failed for '%s': %s",
            gdb_path.resolve(),
            msg,
            exc_info=True,
        )
        raise RuntimeError(
            f"CreateFileGDB failed for '{gdb_path.resolve()}': {msg}"
        ) from None
    except Exception as unexpected_gdb_error:
        log.error(
            "❌ Unexpected error during arcpy.management.CreateFileGDB for '%s': %s",
            gdb_path.resolve(),
            unexpected_gdb_error,
            exc_info=True,
        )
        raise RuntimeError(
            f"Unexpected error during CreateFileGDB for '{gdb_path.resolve()}': {unexpected_gdb_error}"
        ) from unexpected_gdb_error
