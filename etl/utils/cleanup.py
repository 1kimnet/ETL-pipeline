...
def cleanup_downloads_folder() -> int:
    ...
    log.info(
        "🧹 Cleaning downloads folder: %s (%d files)",
        downloads_dir,
        items_before)
...
def cleanup_staging_folder() -> int:
    ...
    log.info(
        "🧹 Cleaning staging folder: %s (%d files)",
        staging_dir,
        items_before)
...
def cleanup_before_pipeline_run(
        clean_downloads: bool = True,
        clean_staging: bool = True) -> None:
    ...
    if total_cleaned > 0:
        lg_sum.info(
            "🧹 Pre-pipeline cleanup complete: %d items removed",
            total_cleaned)
    else:
