...
class ThreadSafeConcurrentDownloader:
    ...
    def download_layers_concurrent(
        ...
        return self._execute_concurrent_tasks(
            tasks, task_names, effective_config)
    ...
    def download_collections_concurrent(
        ...
        log.info(
            "Starting concurrent download of %d collections with %d workers",
            len(collections),
            effective_config.max_workers)
        ...
        with ThreadPoolExecutor(max_workers=config.max_workers) as executor:
            ...
            for future in as_completed(future_to_task, timeout=config.timeout):
                try:
                    ...
                    log.error(
                        "Task %s failed with exception: %s", task_name, e)

        # Sort results by original task order
        results.sort(
            key=lambda r: task_names.index(
                r.metadata.get(
                    "task_name", "")))
...
def create_layer_downloader(
        max_workers: int = 5) -> ThreadSafeConcurrentDownloader:
...
def create_collection_downloader(
        max_workers: int = 3) -> ThreadSafeConcurrentDownloader:
    """Create a downloader optimized for OGC API collections."""
    config = ConcurrentConfig(max_workers=max_workers, timeout=600.0)
    return ThreadSafeConcurrentDownloader(config)


def create_file_downloader(
        max_workers: int = 4) -> ThreadSafeConcurrentDownloader:
