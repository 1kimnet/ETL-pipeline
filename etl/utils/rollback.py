...
class RollbackManager:
    ...
    def add_feature_class_deletion(
            self,
            fc_path: str,
            description: Optional[str] = None) -> None:
    ...
    def add_temp_workspace_cleanup(
            self,
            workspace_path: str,
            description: Optional[str] = None) -> None:
    ...
    def execute_rollback(self, reason: str = "Operation failed") -> bool:
        ...
        if not self.rollback_enabled:
            log.info(
                "🚫 Rollback disabled for operation: %s",
                self.operation_name)
            return True

        if not self.actions:
            log.debug(
                "ℹ️ No rollback actions to execute for: %s",
                self.operation_name)
            return True

        log.info(
            "🔄 Starting rollback for '%s': %s",
            self.operation_name,
            reason)
    ...
    def clear_actions(self) -> None:
        ...
        with self._lock:
            log.debug("🗑️ Cleared %d rollback actions for: %s",
    ...
    def _safe_workspace_cleanup(workspace_path: str) -> None:
        ...
        if arcpy.Exists(workspace_path):
            try:
                ...
                log.warning(
                    "⚠️ Failed to clean workspace %s: %s",
                    workspace_path,
                    e)
...
class TransactionalOperation:
    ...
    def commit(self) -> None:
        ...
        self.completed = True
        log.debug(
            "✅ Committed transactional operation: %s",
            self.operation_name)
    ...
    def rollback(self, reason: str = "Manual rollback") -> None:
        ...
        if not self.started:
            log.warning(
                "⚠️ Attempting to rollback operation that was not started: %s",
                self.operation_name)
            return
        ...
        self.completed = True
        log.debug(
            "🔄 Rolled back transactional operation: %s",
            self.operation_name)
