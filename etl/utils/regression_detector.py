...
class RegressionDetection:
    ...
    def is_significant(self) -> bool:
        ...
        return self.confidence_level > 0.95 and abs(
            self.regression_magnitude) > 0.1
...
class StatisticalAnalyzer:
    ...
    @staticmethod
    def calculate_regression_magnitude(
            baseline: float,
            current: float) -> float:
...
class PerformanceRegressionDetector:
    ...
    def record_performance(
            self, metrics: PerformanceMetrics) -> Optional[List[RegressionDetection]]:
    ...
    def establish_baseline(
        ...
        if len(data) < self.min_samples_for_baseline:
            log.warning(
                "Insufficient data for baseline: %s (%d samples)",
                operation,
                len(data))
            return False
    ...
    def get_regression_summary(self) -> Dict[str, Any]:
        ...
        return {
            ...
            "by_operation": {
                op: len(regressions) for op,
                regressions in by_operation.items()},
            "recent_regressions": [
    ...
    def generate_regression_report(
            self, hours_back: int = 24) -> Dict[str, Any]:
    ...
    def _should_update_baseline(self, operation: str) -> bool:
        ...
        if operation not in self.baselines:
            return len(
                self.performance_data[operation]) >= self.min_samples_for_baseline
    ...
    def _update_baseline(self, operation: str) -> bool:
        ...
        log.info(
            "Updated baseline for %s with %d samples",
            operation,
            len(baseline_data))
        return True
    ...
    def _detect_regressions(
            self,
            operation: str,
            metrics: PerformanceMetrics) -> List[RegressionDetection]:
    ...
    def _calculate_stability_score(
            self, data: List[PerformanceMetrics]) -> float:
    ...
    def _generate_regression_recommendations(
            self, regressions: List[RegressionDetection]) -> List[str]:
                ...
                if regressions:
                    for regression in regressions:
                        if regression.is_significant:
                            log.warning(
                                "🔍 Performance regression detected in %s", operation_name)
