...
class CircuitBreaker:
        ...
        with self._lock:
            ...
            if self.state == CircuitBreakerState.HALF_OPEN:
                if self.half_open_calls >= self.half_open_max_calls:
                    ...
                    log.info("✅ Circuit breaker '%s' reset to CLOSED after successful test", self.name)
    ...
    def _on_failure(self, exception: Exception) -> None:
        ...
        with self._lock:
            ...
            if self.state == CircuitBreakerState.HALF_OPEN:
                ...
                log.warning("🔴 Circuit breaker '%s' reopened after failure during test", self.name)
            ...
            log.debug("❌ Circuit breaker '%s' recorded failure: %s", self.name, exception)
    ...
    def _transition_to_half_open(self) -> None:
        ...
        self.stats.state_changes += 1
        self.stats.last_state_change_time = time.time()
        log.info("🔄 Circuit breaker '%s' transitioned to HALF_OPEN for testing", self.name)
