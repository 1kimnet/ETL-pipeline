...
def _parse_filename_from_content_disposition(
        header_value: str) -> Optional[str]:
    """
    ...
    if match:
        ...
        if "UTF-8''" in match.group(0).lower():  # Check if it was filename*
            ...
            # by the server sending a raw URL part. Unquoting here is generally
            # safe.
            return unquote(potential_filename)
    return None


def fetch_true_filename_parts(
        download_url: str, timeout: int = 10) -> Tuple[str, str]:
    """
    ...
    log.debug(
        "Attempting to determine true filename for URL: %s",
        download_url)
        ...
        with urlopen(req, timeout=timeout) as resp:  # Potentially add context=context here
            ...
            if cd_header:
                ...
                if true_filename_str:
                    log.info(
                        "    Derived filename from Content-Disposition: %s",
                        true_filename_str)
    ...
    # 2. Fallback to unquoting URL basename (from final URL after potential
    # redirects)
    if not true_filename_str:
        ...
        if url_path_name:
            ...
            log.info(
                "    Derived filename by unquoting URL basename ('%s'): %s",
                final_url_after_redirects,
                true_filename_str)
        else:
            log.warning(
                "    Could not derive filename from URL path for %s",
                final_url_after_redirects)
