    ...
    def get_session(self) -> requests.Session:
        ...
        if not hasattr(self._local, 'session'):
            ...
            retry_strategy = Retry(
                total=self.max_retries, backoff_factor=self.backoff_factor, status_forcelist=[
                    429, 500, 502, 503, 504], allowed_methods=[
            ...
            log.debug(
                "Created new HTTP session for thread %s",
...
class ResponseCache:
    ...
    def __init__(
        ...
        log.info(
            "💾 Initialized response cache: max_size=%d, ttl=%ds",
            max_size,
            default_ttl)
    ...
    def set(
        ...
        try:
            if hasattr(
                    data, '__len__') and len(
                    str(data)) > self.max_response_size:
                log.debug(
                    "Response too large to cache: %d bytes", len(
                        str(data)))
                return
        except Exception:
...
class MemoryManager:
    ...
    def get_optimal_chunk_size(self, total_size: int) -> int:
        ...
        # Optimize chunk size to minimize number of chunks while staying under
        # limit
    ...
    def process_in_chunks(
        ...
        log.debug(
            "Processing %d items in chunks of %d",
            total_size,
            chunk_size)
    ...
    def stream_file_chunks(
        ...
        try:
            ...
            raise ResourceError(
                f"File streaming failed: {e}",
                resource_type="file_io") from e
...
class ParallelProcessor:
    ...
    def __init__(self, max_workers: Optional[int] = None):
        ...
        log.info(
            "⚡ Parallel processor initialized: max_workers=%s",
            max_workers or "auto")
    ...
    def process_sources_parallel(
        ...
        log.info(
            "🚀 Processing %d sources in parallel (workers=%s)",
            len(sources),
            workers)
        ...
        with ThreadPoolExecutor(max_workers=workers) as executor:
            ...
            for future in as_completed(future_to_source):
                ...
                try:
                    ...
                    log.debug(
                        "✅ Completed processing source: %s", getattr(
                            source, 'name', str(source)))
                except Exception as e:
                    self._metrics.errors += 1
                    results.append((source, e))
                    log.error(
                        "❌ Failed processing source %s: %s", getattr(
                            source, 'name', str(source)), e)
