...
class GlobalCache:
    ...
    def get_stats(self):
        return {"performance": {"hit_rate_percent": 75}}


class GlobalTuner:
    """Placeholder for global tuner."""
...
def get_global_tuner() -> GlobalTuner:
...
class PerformanceMonitor:
        ...
        if relevant_system_metrics:
            system_metrics = {
                "avg_cpu_percent": statistics.mean(
                    m.cpu_percent for m in relevant_system_metrics
                ),
                "avg_memory_percent": statistics.mean(
                    m.memory_percent for m in relevant_system_metrics
                ),
                "min_disk_free_gb": min(
                    m.disk_free_gb for m in relevant_system_metrics
                ),
                "pressure_periods": sum(
    ...
    def _summarize_operation_metrics(
        self, operation: str, metrics: List[PerformanceMetrics]
    ) -> Dict[str, Any]:
        ...
        return {
            ...
            "efficiency": {
                "avg_items_per_mb": statistics.mean(
                    m.memory_efficiency for m in metrics
                ),
