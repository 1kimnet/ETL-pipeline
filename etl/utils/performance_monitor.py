...
class PerformanceMonitor:
    ...
    def start_monitoring(self) -> None:
        ...
        log.info("🔍 Started performance monitoring (interval: %.1fs)", self.monitoring_interval)
    ...
    def generate_report(self, hours_back: int = 1) -> PerformanceReport:
        ...
        if relevant_system_metrics:
            system_metrics = {
                "avg_cpu_percent": statistics.mean(m.cpu_percent for m in relevant_system_metrics),
                "avg_memory_percent": statistics.mean(m.memory_percent for m in relevant_system_metrics),
