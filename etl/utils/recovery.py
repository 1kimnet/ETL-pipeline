...
class RecoveryManager:
        ...
        for strategy in strategies_to_try:
            try:
                ...
                if result.success:
                    ...
                    log.info(
                        "✅ Recovery successful using %s strategy",
                        strategy.strategy.value)
                    return result
    ...
    def _handle_partial_strategy(
        ...
        log.info(
            "🔄 Continuing with partial results for: %s",
            operation_context)
    ...
    def _handle_degrade_strategy(
        ...
        if self.degradation_level >= self.max_degradation_level:
            log.error(
                "❌ Maximum degradation level reached, cannot degrade further")
            return RecoveryResult(
                success=False,
        ...
        log.warning("⬇️ Degrading service level to %d for: %s",
    ...
    def reset_degradation_level(self) -> None:
        ...
        if old_level > 0:
            log.info(
                "⬆️ Service degradation level reset from %d to 0",
                old_level)
...
def graceful_degradation(
    ...
    try:
        ...
        log.warning(
            "⚠️ Operation '%s' failed, attempting recovery",
            operation_name)
        ...
        if recovery_result.success:
            ...
            log.error(
                "❌ Recovery failed for '%s', re-raising exception",
                operation_name)
            raise
    ...
    def decorator(func: Callable[..., T]) -> Callable[..., T]:
        def wrapper(*args, **kwargs) -> T:
            ...
            try:
                ...
                log.warning(
                    "⚠️ Function '%s' failed, attempting recovery",
                    operation_name)
...
class GracefulDegradationConfig:
    ...
    def get_degraded_config(self, level: int) -> Dict[str, Any]:
        ...
        return self.degradation_thresholds.get(
            level, self.degradation_thresholds[3])
