...
class AtomFeedDownloadHandler: 
    ...
    def _download_and_stage_linked_resource(
        ...
        try:
            ...
            # Determine how to stage based on the *actual downloaded file type* (ZIP or GPKG)
            # and the source's overall staged_data_type expectation.
            ...
            if is_downloaded_zip:
                ...
                try:
                    ...
                    # If the source expects a GPKG, and we just extracted a ZIP,
                    # look for a GPKG inside the extracted contents.
                    if self.src.staged_data_type == "gpkg":
                        ...
                        if extracted_gpkgs:
                            ...
                            # Rename/move the found GPKG to the expected name for the loader
                            # Expected name: sanitized_atom_source_name.gpkg
                            ...
                            if extracted_gpkgs[0].name != expected_staged_gpkg_name:
                                ...
                                logging.info("            Extracted GPKG '%s' already has the expected name. [V1.2 LOG]", final_staged_gpkg_path.name)
                ...
                # The downloaded link was directly a GPKG file.
                # Stage it by copying it to target_staging_dir, named as sanitized_atom_source_name.gpkg
                staged_gpkg_filename: str = sanitized_atom_source_name + ".gpkg" # Consistent name
            ...
            else: # Neither ZIP nor GPKG, and not handled by a specific source.staged_data_type
                logging.warning(
                    f"        🤷 Downloaded file '{downloaded_file_path.name}' from Atom link is not a recognized ZIP or GPKG. "
                    f"Source type is '{self.src.staged_data_type}'. Staging not performed for this link. [V1.2 LOG]"
                )
