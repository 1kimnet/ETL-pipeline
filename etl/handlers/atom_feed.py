...
class AtomFeedDownloadHandler:
    ...
    def fetch(self) -> None:
        if not self.src.enabled:
            log.info("⏭️ Source '%s' (Atom Feed) is disabled, skipping fetch.", self.src.name)
            return

        log.info("🌐 Reading Atom feed for source '%s' from URL: %s", self.src.name, self.src.url)
        ...
        try:
            ...
            log.error("❌ Failed to download or read Atom feed XML for '%s' from %s: %s",
        ...
        try:
            ...
            log.error("❌ Malformed Atom XML for source '%s': %s", self.src.name, exc, exc_info=True)
            return
    ...
    def _download_and_stage_linked_resource(
        ...
        try:
            ...
            log.info("Attempting to download linked resource: %s \n        -> to local file: %s",
            ...
            if is_downloaded_zip:
                ...
                try:
                    ...
                    log.info("➕ Extracted and staged archive %s into %s",
                             downloaded_file_path.name, target_staging_dir.relative_to(paths.ROOT))
                    ...
                    if self.src.staged_data_type == "gpkg":
                        ...
                        if extracted_gpkgs:
                            ...
                            log.info("✅ Successfully staged GPKG '%s' for source '%s'.",
                                     final_staged_gpkg_path.name, self.src.name)
                        else:
                            log.warning("⚠️ Source '%s' expected a GPKG, but no .gpkg file found after extracting %s.",
                                      self.src.name, downloaded_file_path.name)
                except zipfile.BadZipFile:
                    log.error("❌ File '%s' from Atom link is not a valid ZIP file. Cannot extract.",
                ...
                try:
                    if downloaded_file_path.resolve() != staged_gpkg_path.resolve():
                        ...
                        log.info("➕ Staged direct GPKG %s to %s",
                ...
                log.warning(
                    ...
                    downloaded_file_path.name, self.src.staged_data_type
                )
