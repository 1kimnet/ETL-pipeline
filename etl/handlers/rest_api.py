...
class RestApiDownloadHandler:
    ...
    def _get_service_metadata(self, service_url: str) -> Optional[Dict[str, Any]]:
        ...
        try:
            ...
            total_attempts = 1
            log.warning("Invalid value for 'max_retries' in global_config. Defaulting to 1 attempt for metadata fetch.")

        for attempt in range(total_attempts):
            try:
                log.debug("Attempt %d/%d to fetch service metadata from %s", attempt + 1, total_attempts, service_url)
                response = requests.get(service_url, params=params, headers=headers, timeout=30)
                response.raise_for_status()
                return response.json()
            except requests.exceptions.HTTPError as e:
                log.error("❌ HTTP error fetching service metadata from %s (Attempt %d/%d): %s",
                         service_url, attempt + 1, total_attempts, e)
                if 400 <= e.response.status_code < 500:
                    log.warning("Client error %d, not retrying metadata fetch for %s.",
                               e.response.status_code, service_url)
                    return None
                if attempt + 1 == total_attempts:
                    log.error("Final attempt failed for %s with HTTP error.", service_url)
                    return None
                sleep_time = 5 * (attempt + 1)
                log.info("Server error. Retrying metadata fetch for %s in %ds...", service_url, sleep_time)
                time.sleep(sleep_time)
            except requests.exceptions.RequestException as e:
                log.error("❌ Request exception fetching service metadata from %s (Attempt %d/%d): %s",
                         service_url, attempt + 1, total_attempts, e)
                if attempt + 1 == total_attempts:
                    log.error("Final attempt failed for %s with RequestException.", service_url)
                    return None
                sleep_time = 5 * (attempt + 1)
                log.info("Request exception. Retrying metadata fetch for %s in %ds...", service_url, sleep_time)
                time.sleep(sleep_time)
            except Exception as e:
                log.error("❌ Unexpected error during metadata fetch for %s (Attempt %d/%d): %s",
                         service_url, attempt + 1, total_attempts, e, exc_info=True)
                return None
        
        log.error("All %d attempts to fetch metadata from %s failed.", total_attempts, service_url)
        return None
    ...
    def fetch(self) -> None:
        ...
        if not self.src.enabled:
            log.info("⏭️ Source '%s' (REST API) is disabled, skipping fetch.", self.src.name)
            return

        log.info("🌐 Processing REST API source: '%s' from URL: %s", self.src.name, self.src.url)

        service_meta = self._get_service_metadata(self.src.url)
        if not service_meta:
            log.error("❌ Could not retrieve service metadata for %s. Skipping source.", self.src.name)
            return
        ...
        if configured_layer_ids_from_yaml:
            log.info("Found explicit layer_ids in config: %s for source '%s'. Processing only these.",
                     configured_layer_ids_from_yaml, self.src.name)
        ...
        # Fallback for single-layer FeatureServer
            ...
            log.info("Source '%s' appears to be a single-layer FeatureServer and no layers were previously identified. "
        ...
        if not layers_to_iterate_final:
            log.warning("⚠️ No layers identified or specified to query for source '%s'. "
        ...
        log.info("Source '%s': Will attempt to query %d layer(s): %s",
    ...
    def _fetch_layer_data(self, layer_info: Dict[str, Any], layer_metadata_from_service: Optional[Dict[str, Any]] = None) -> None:
        ...
        log.info("Querying Layer ID: %s (Sanitized Name: %s, Original: %s) from %s",
        ...
        if max_record_count_from_config is not None:
            try:
                ...
                log.info("Using max_record_count from source.raw config: %d", max_record_count)
            except ValueError:
                log.warning("Invalid 'max_record_count' in source.raw: '%s'. Falling back to metadata or default.",
        ...
        if max_record_count is None:
            if not layer_meta_to_use:
                ...
                log.debug("Fetching specific layer metadata for layer ID %s to determine maxRecordCount.", layer_id)
            ...
            if layer_meta_to_use:
                if layer_meta_to_use.get("maxRecordCount") is not None:
                    ...
                    log.info("Service layer metadata indicates maxRecordCount: %d", max_record_count)
                elif layer_meta_to_use.get("standardMaxRecordCount") is not None:
                    max_record_count = layer_meta_to_use["standardMaxRecordCount"]
                    log.info("Service layer metadata indicates standardMaxRecordCount: %d", max_record_count)
                else:
                    max_record_count = 2000
                    log.info("maxRecordCount not found in specific layer metadata, using default: %d", max_record_count)
            else:
                max_record_count = 2000
                log.warning("Could not fetch specific layer metadata for maxRecordCount, using default: %d", max_record_count)
        
        # Ensure max_record_count is an integer after all determinations
        if not isinstance(max_record_count, int):
            log.warning("max_record_count ended up non-integer: '%s'. Defaulting to 2000.", max_record_count)
            max_record_count = 2000
        ...
        while True:
            ...
            log.info("Fetching page %d for layer %s (offset %d, limit %d)", 
            ...
            try:
                ...
                if "error" in data:
                    log.error("❌ API_ERROR_REPORTED: Error from REST API for layer %s: %s", 
                             layer_name_sanitized, data["error"])
                    log.error("❌ API_ERROR_REPORTED: Breaking from pagination loop for this layer.")
                    break 

                features = data.get("features", [])
                if not features:
                    if page_num == 1:
                        log.info("ℹ️ No features returned for layer %s with current parameters.", layer_name_sanitized)
                    else:
                        log.info("🏁 All features retrieved for layer %s (empty page).", layer_name_sanitized)
                    break
                ...
                if exceeded_transfer_limit:
                    log.info("⚠️ Exceeded transfer limit for layer %s, fetching next page.", layer_name_sanitized)
                    current_offset += len(features) 
                    page_num += 1
                elif (len(features) < effective_page_limit and effective_page_limit > 0) or max_record_count == 0:
                    log.info("🏁 All features likely retrieved for layer %s (less than page limit or server maxRecordCount is 0).", 
                ...
                log.error("❌ PAGINATION_LOOP_REQUEST_ERROR: Failed to download data for layer %s, page %d: %s", 
                         layer_name_sanitized, page_num, e)
                log.error("❌ PAGINATION_LOOP_REQUEST_ERROR: Breaking from pagination loop for this layer.")
                break 
            except json.JSONDecodeError as e:
                log.error("❌ PAGINATION_LOOP_JSON_ERROR: Failed to decode JSON for layer %s, page %d: %s", 
                         layer_name_sanitized, page_num, e)
                if response_obj:
                    log.debug("Raw response text for JSON error: %s", response_obj.text[:500])
                log.error("❌ PAGINATION_LOOP_JSON_ERROR: Breaking from pagination loop for this layer.")
                break
            except Exception as e_unexpected: 
                log.error("❌ PAGINATION_LOOP_UNEXPECTED_ERROR: Unexpected error for layer %s, page %d: %s", 
                         layer_name_sanitized, page_num, e_unexpected, exc_info=True)
                log.error("❌ PAGINATION_LOOP_UNEXPECTED_ERROR: Breaking from pagination loop for this layer.")
                break
        ...
        if all_features:
            ...
            if params['f'] == 'geojson':
                if not layer_meta_to_use: 
                    layer_metadata_url = f"{self.src.url.rstrip('/')}/{layer_id}"
                    log.debug("Fetching specific layer metadata for layer ID %s (for CRS info).", layer_id)
                    layer_meta_to_use = self._get_layer_metadata(layer_metadata_url)
            ...
            try:
                ...
                log.info("💾 Successfully saved %d features for layer %s to %s",
                ...
                log.error("❌ Failed to write data for layer %s to %s: %s", layer_name_sanitized, output_path, e)
        elif page_num == 1 and not all_features:
            log.info("ℹ️ No features found or written for layer %s for source '%s'.", layer_name_sanitized, self.src.name)