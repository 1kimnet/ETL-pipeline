...
def process_geojson_file(
    ...
    log.debug("🌍 Processing GeoJSON - Authority: '%s' for file: %s", authority, json_file_path.name)
    ...
    try:
        ...
        log.error("❌ JSONToFeatures failed for %s → %s: %s. ArcPy Messages: %s",
        ...
        log.error("❌ Unexpected error processing JSON/GeoJSON %s: %s",
                  json_file_path.name, generic_error, exc_info=True)
        summary.log_staging("error")
