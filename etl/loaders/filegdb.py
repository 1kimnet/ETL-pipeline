...
class ArcPyFileGDBLoader:
        ...
        if not self.gdb_path.exists():
            ...
            try:
                arcpy.management.CreateFileGDB(
                    str(self.gdb_path.parent), self.gdb_path.name
                )
    ...
    def _process_configured_sources(
        ...
        for source in self.sources:
            ...
            try:
                log.info(
                    "🔄 Processing source: %s (Authority: %s)",
                    source.name,
                    source.authority,
                )
    ...
    def _normalize_staged_data_type(
        self, staged_data_type: Optional[str]
    ) -> Optional[str]:
    ...
    def _handle_gpkg_source(
        ...
        if not source_staging_dir.exists():
            log.warning(
                "⚠️ Staging directory not found for GPKG source '%s': %s",
                source.name,
                source_staging_dir,
            )
            return
        ...
        expected_gpkg_path = (
            source_staging_dir / f"{sanitize_for_filename(source.name)}.gpkg"
        )
        ...
        if expected_gpkg_path.exists():
            ...
            if gpkg_files:
                log.info(
                    "📦 Found %d GPKG file(s) for source '%s'",
                    len(gpkg_files),
                    source.name,
                )
                ...
                log.warning(
                    "⚠️ No GPKG file found for source '%s' in dir %s",
                    source.name,
                    source_staging_dir,
                )
    ...
    def _handle_geojson_source(
        ...
        if not source_staging_dir.exists():
            log.warning(
                "⚠️ Staging directory not found for JSON/GeoJSON source '%s': %s",
                source.name,
                source_staging_dir,
            )
            return
        ...
        if all_json_files:
            log.info(
                "🌍 Found %d JSON/GeoJSON file(s) for source '%s'.",
                len(all_json_files),
                source.name,
            )
    ...
    def _handle_shapefile_source(
        ...
        if source.include and source.type == "file":
            self._process_multi_part_shapefile_collection(
                source, staging_root, used_names_set
            )
        else:
            self._process_single_shapefile_source(
    ...
    def _process_multi_part_shapefile_collection(
        ...
        for included_item_stem in source.include or []:
            ...
            if not item_staging_dir.exists():
                log.warning(
                    "⚠️ Staging subdirectory not found for item '%s': %s",
                    included_item_stem,
                    item_staging_dir,
                )
                continue
    ...
    def _process_single_shapefile_source(
        ...
        if not source_staging_dir.exists():
            log.warning(
                "⚠️ Staging directory not found for source '%s': %s",
                source.name,
                source_staging_dir,
            )
            return
    ...
    def _glob_and_load_geojsonfiles(
        self, staging_root: Path, used_names_set: Set[str]
    ) -> None:
        ...
        if all_json_files:
            log.info(
                "🔍 Fallback: Found %d JSON/GeoJSON file(s) to process.",
                len(all_json_files),
            )
            for json_file_path in all_json_files:
                derived_authority = derive_authority_from_path(
                    json_file_path, staging_root
                )
                process_geojson_file(
                    json_file_path,
                    derived_authority,
...
def load_from_staging(
    ...
    loader = ArcPyFileGDBLoader(
        summary=summary, gdb_path=gdb_path, sources_yaml_path=sources_yaml_path
    )
