...
class TestSanitizeForArcgisName:
    ...
    def test_arcgis_real_world_examples(self):
        assert sanitize_for_arcgis_name("Naturvårdsverket") == "naturvardsverket"
        assert sanitize_for_arcgis_name("Försvarsmakten - Geodata") == "forsvarsmakten_geodata"
...
class TestGenerateFcName:
    ...
    def test_fc_name_real_world_examples(self):
        ...
        assert generate_fc_name("FM", "Rikstäckande geodata") == "FM_rikstackande_geodata"
        assert generate_fc_name("NVV", "Naturvårdsregistret") == "NVV_naturvardsregistret"
